...
export default function AddItemNeeded({ isOpen, onClose, documentsNeeded, onSuccess }: AddItemNeededProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newBulletPoint = `* ${itemText.trim()}`
      
      // Build the updated documentsNeeded string, preserving exact format
      let updatedDocumentsNeeded: string
      
      if (!documentsNeeded || documentsNeeded.trim() === '') {
        // If empty, just add the new bullet point
        updatedDocumentsNeeded = newBulletPoint
      } else {
        // Split by newlines to preserve exact format
        const lines = documentsNeeded.split('\n')
        ...
        if (lastNonEmptyIndex === -1) {
          ...
          updatedDocumentsNeeded = updatedLines.join('\n')
        }
      }
      ...
      const data = await response.json().catch(() => ({ detail: 'Failed to add item' }))
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-2 duration-300 flex flex-col">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-100 mt-0.5">
                  Add an item to your documents needed list
                </p>
              </div>
            </div>
            <button
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          <div>
            <label htmlFor="item-text" className="block text-sm font-semibold text-gray-900 mb-2">
              Item Text
            </label>
            <input
              id="item-text"
              ...
              placeholder="e.g., Driver's License, Insurance Card, Registration"
              disabled={loading}
            ...
            />
            <p id="item-text-help" className="mt-2 text-xs text-gray-500 flex items-center gap-1.5">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              The item will be added as a bullet point to your documents needed list.
            </p>
          </div>
