...
import AddAuctionTrigger from './AddAuctionTrigger'

interface AuctionTriggersCardProps {
  title: string
  description?: string
...
export default function AuctionTriggersCard({ title, description, auctionTriggers = '', onUpdate }: AuctionTriggersCardProps) {
  ...
  const handleDelete = async (lineIndex: number) => {
    ...
    const currentAuctionTriggers = auctionTriggers || ''
    if (!currentAuctionTriggers.trim()) {
      setError('No triggers to delete')
      return
    }
    ...
    try {
      ...
      if (!session) {
        setError('You must be signed in to delete triggers')
        setLoading(prev => ({ ...prev, [lineIndex]: false }))
        return
      }
      ...
      const lines = currentAuctionTriggers.split('\n')
      ...
      if (targetLineIndex === -1) {
        setError('Could not find the trigger to delete')
        setLoading(prev => ({ ...prev, [lineIndex]: false }))
        return
      }
      ...
      const updatedAuctionTriggers = remainingBulletPoints.length === 0 
        ? '' 
        : updatedLines.join('\n')
      ...
      if (!response.ok) {
        const data = await response.json().catch(() => ({ detail: 'Failed to delete trigger' }))
  ...
  const bulletLines = auctionTriggers
    .split('\n')
      ...
      <div className={description ? 'mb-6' : 'mb-7'}>
        <div className="flex items-start justify-between gap-6">
          ...
          <button
            ...
            title="Add new auction trigger"
            aria-label="Add new auction trigger"
          >
            <svg
              className="w-5 h-5"
              fill="none"
        ...
        {bulletLines.length > 0 ? (
          <div className="grid grid-cols-1 gap-3">
            {bulletLines.map((line, index) => {
              const triggerText = line.trim().replace(/^\*\s*/, '')
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    title={isLoading ? 'Deleting...' : 'Delete trigger'}
                    aria-label={isLoading ? 'Deleting trigger' : 'Delete trigger'}
                  >
                    {isLoading ? (
                      <svg
          ...
          <div className="flex flex-col items-center justify-center py-12 px-4 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
            ...
            <p className="text-xs text-gray-500 text-center max-w-sm">
              Click the "Add Trigger" button above to get started
            </p>
          </div>
        )}
      </div>
