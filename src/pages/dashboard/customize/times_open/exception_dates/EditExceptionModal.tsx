...
// Parse hours string (e.g., "4:00 AM - 7:00 PM, 8:00 AM - 9:00 PM" or "Closed")
function parseHoursString(hours: string): { ranges: TimeRange[]; isClosed: boolean } {
  if (!hours || typeof hours !== 'string') {
    return { ranges: [{ start: '04:00', end: '19:00' }], isClosed: false }
  }

  if (hours.trim().toLowerCase() === 'closed') {
    return { ranges: [], isClosed: true }
  }
  ...
  for (const part of parts) {
    ...
    if (match) {
      ...
      try {
        const start = formatTime12To24(
          parseInt(startHour, 10),
          parseInt(startMin, 10),
          startPeriod.toUpperCase() as 'AM' | 'PM'
        )
        const end = formatTime12To24(
          parseInt(endHour, 10),
          parseInt(endMin, 10),
          endPeriod.toUpperCase() as 'AM' | 'PM'
        )
        ranges.push({ start, end })
      } catch (err) {
        // Skip invalid time ranges
        console.warn('Failed to parse time range:', part, err)
      }
    }
  }

  // If no ranges were parsed, default to a single range
  if (ranges.length === 0) {
    ranges.push({ start: '04:00', end: '19:00' })
  }

  return { ranges, isClosed: false }
}
...
export default function EditExceptionModal({ 
  ...
  // Parse and set initial state when modal opens
  useEffect(() => {
    if (isOpen) {
      const parsed = parseHoursString(currentHours)
      setIsClosed(parsed.isClosed)
      // Only set default range if not closed and no ranges were parsed
      if (!parsed.isClosed) {
        setRanges(parsed.ranges.length > 0 ? parsed.ranges : [{ start: '04:00', end: '19:00' }])
      } else {
        setRanges([])
      }
      setError(null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!session) {
        setError('You must be signed in to update exception dates')
        setLoading(false)
        return
      }
      ...
      if (!response.ok) {
        setError(data.detail || 'Failed to update exception date')
        setLoading(false)
        return
      }
